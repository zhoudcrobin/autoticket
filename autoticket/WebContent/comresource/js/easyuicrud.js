   ...
   toolMap.put("删除", "{text:'删除',iconCls:'icon-remove',handler:'delCallBack'}");
	...
	this.openDataGrid = function(datagridID,options){
		...
		$(datagridID).datagrid({
		    ...
		    toolbar:(options.toolbar ? options.toolbar : toolArr),
	...
	this.delFrozenItem = function(text){
		frozenMap.remove(text);
	}
}
   ...
   queryWinID = '#query-window';
	...
	this.updOperateBack = function(iframeID,options){
	    ...
	    callBackId=function(row){
	        return row[options.idname?options.idname:'id'];
	    }
	...
	/*删除操作*/ 
	this.delOperateBack = function(idname){		
		if(!isExistID(datagridID))return;
	    ...
	    var ids = '';
	    if(typeof(idname) == 'undefined'|| idname == '')idname = 'id';
	    for(var i=0;i<rows.length;++i){
	        ids =ids + 'selections=' + rows[i][idname] +'&';
	    }
